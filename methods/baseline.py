...
class BASELINE(ER):
    ...
    def online_train(self, sample, batch_size, n_worker, iterations=1, stream_batch_size=0):
        ...
        for i in range(iterations):
            ...
            logit, loss, features = self.model_forward(x, y, get_feature=True)
            '''
            unique_y = torch.unique(y).tolist()
            for u_y in unique_y:
                indices = (y == u_y.item()).nonzero(as_tuple=True)[0]
                torch.index_select(features, 0, indices)
                self.feature_dict[u_y.item()]
            '''
